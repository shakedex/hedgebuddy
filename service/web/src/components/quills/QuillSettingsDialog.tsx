...
import { fetchQuillSettings, saveQuillSettings, testQuillConnection } from '#/lib/api'
          ...
          <div className="space-y-4 py-2">
            {(quill.settings ?? []).map((def) => (
              <div key={def.name} className="space-y-1.5">
                ...
                <Input
                  type={def.type === 'secure' ? 'password' : 'text'}
                  value={values[def.name] ?? ''}
                  onChange={(e) => setValues({ ...values, [def.name]: e.target.value })}
                  ...
                  <span className="flex items-center gap-1 text-xs">
                    {testResult.ok ? (
                      ...
                      <>
                        ...
                        <span className="text-destructive truncate max-w-[200px]" title={testResult.error}>
