 ...
 */
export function VarEditor({ id, type, value, onChange, error, secret }: {
  id: string;
  type: VarType;
  value: EditValue;
  ...
  if (type === "secret") {
    if (!secret) {
      if (import.meta.env.DEV) {
        console.warn(`VarEditor: type "secret" needs the "secret" prop (id "${id}")`);
      }
      return null;
    }
    return (
    ...
    return (
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-2">
          <Switch id={id} checked={checked} onCheckedChange={onChange} aria-invalid={!!error} aria-describedby={error ? errorId : undefined} />
  ...
  const mono = type === "url" || numeric;
  return (
    <div className="flex flex-col gap-1.5">
      <Input
        id={id}
        className={cn(type === "url" && "font-mono", numeric && "max-w-48 font-mono readout text-right")}
